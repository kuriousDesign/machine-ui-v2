...
import { getCachedPollingTagCount, getTagTopicsCount, getTagTopicsPayload } from "@/lib/bridge/types";
import { useBridgeState } from "@/lib/store/zustand-provider";

export function TagTopicsView() {
    ...
    <div className="space-y-4">
      <SectionCard title="GET_CACHE Snapshot" description="Heavy metadata lives here instead of bridge/status, including TagTopics diagnostics.">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Metric label="Cached topics" value={String(getCachedPollingTagCount(cacheSnapshot))} />
          <Metric label="TagTopics entries" value={String(getTagTopicsCount(cacheSnapshot))} />
                              ...
                              {sourceGroup.items.map((item) => (
                                <div key={item.id} className="grid gap-2 px-4 py-3 md:grid-cols-[auto_1fr_auto] md:items-center">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs uppercase tracking-[0.18em] text-[var(--muted)]">{item.statusLabel}</span>
                                  </div>
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    {item.reason ? (
                                      <p className="mt-1 line-clamp-2 text-xs text-amber-700">Reason: {item.reason}</p>
                                    ) : null}
                                  </div>
...
function StatusDot({ isHealthy }: { isHealthy: boolean }) {
  return <span aria-hidden className={`inline-block h-3 w-3 rounded-full ${isHealthy ? "bg-emerald-500" : "bg-rose-500"}`} />;
}
...
function normalizeTagTopicItem(entry: any, index: number): TagTopicItem | null {
  ...
  const title = getString(item.tagId) ?? getString(item.key) ?? getString(item.nodeId) ?? getString(item.mqttTopic) ?? `Entry ${index + 1}`;
...
function inferLifecycleStage(source: string, readStatus: string, pollStatus: string): TagTopicItem["lifecycleStage"] {
...
function isHealthyTag(item: Record<string, unknown>, readStatus: string, pollStatus: string): boolean {
  const normalizedRead = readStatus.toLowerCase();
  const normalizedPoll = pollStatus.toLowerCase();
    ...
    .map((scope) => {
      ...
      const stageGroups = stageOrder
        .map((stage) => {
          ...
          return {
            items: stageItems,
            label: stage === "bootstrap" ? "Bootstrap tags" : stage === "monitored" ? "Monitored tags" : "Other tags",
