...
export function OverviewView() {
  ...
  const activeSubscriptionCount = useBridgeState((state) => state.subscriptions.activeTopics.length);
      ...
      <section className="grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
        <SectionCard title="Machine UI V2" description="A segmented MQTT client for bridge state, cache hydration, and device-scoped Zustand selectors.">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <SummaryStat label="Cache update" value={cacheUpdatedAt ? new Date(cacheUpdatedAt).toLocaleTimeString() : "Pending"} />
          </div>
        </SectionCard>
